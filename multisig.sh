...
txRaw=$(docker exec regtest-alice namecoin-cli -regtest createrawtransaction '[{"txid":"'$txid'","vout":'$vout_id'}]' '{"'$changeAddr'": '$changeAmount', "'$BOB_ADDR'": '$nameAmount'}')
echo "txRaw: "$txRaw
...
txRaw=$(docker exec regtest-alice namecoin-cli -regtest signrawtransaction $hex)
